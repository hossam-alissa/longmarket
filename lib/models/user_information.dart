...
//
//
//       AuthResult _res = await FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: passwordUser);
//
//       try{
...
//       this.city = city;
//       this._token =
//           ((await _res.user.getIdToken().then((value) => (value.token)))
//               .toString());
//       this._expiryDate =
//           DateTime.now().add(Duration(seconds: int.parse("3600")));
//
//       final String url = 'https://long-market-default-rtdb.firebaseio.com/users.json?auth=${this._token};';
//       await http.post(url,
//           body: json.encode({
//             'uid': _res.user.uid,
//             'email': email.toString(),
...
//       // }) ;
//
//       AuthResult _res = await FirebaseAuth.instance.signInWithEmailAndPassword(
//         email: emailUserName,
//         password: passwordUser,
//       );
//       print('+++++ +++++' + "${_res.user.getIdToken()}");
//       print('+++++ +++++' + "${_res.user.isEmailVerified}");
//       print('+++++ +++++' + "${_res.additionalUserInfo}");
//
//       try {
